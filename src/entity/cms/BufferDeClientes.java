...
public class BufferDeClientes implements Buffer<Cliente> {
    ...
    public BufferDeClientes() {
        ...
        this.clientescarregados = new ArrayList<>();
    }
    ...
    public Cliente pesquisarPorNome(String nome){
        //Filtra os cliente para encontrar o nome
        return clientescarregados.stream()
    ...
    public void removerPorNome(String nome) throws IOException {
        if (!modo.equals("escrita")){
            throw new IllegalStateException("Buffer nao respondendo em escrita!");
        }

        clientescarregados = clientescarregados.stream()
                .filter(cliente -> !cliente.getNome().equalsIgnoreCase(nome))
                .collect(Collectors.toList());

        try {
            arquivoSequencial.escreveNoArquivo(clientescarregados);
        } catch (IOException e) {
